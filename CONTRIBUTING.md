...
Maya versions to test against are set in [reusable-maya-tests.yml](.github/workflows/reusable-maya-tests.yml); that job is skipped if `tests` don't exist. Docs deploy on push to main via [docs.yml](.github/workflows/docs.yml), to [https://matthewlee-dev.github.io/locator-creator](https://matthewlee-dev.github.io/locator-creator); enable once via repo Settings > Pages, source: `GitHub Actions`.
